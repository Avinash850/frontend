...
const SpecialityCard = ({ name, description, icon, bgColor }) => (
    <div className="bg-white p-4 sm:p-5 rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4 cursor-pointer">
        <div className={`w-20 h-20 rounded-lg flex-shrink-0 flex items-center justify-center ${bgColor}`}>
            {icon}
        </div>
        <div className="flex-grow">
            <h3 className="font-bold text-lg sm:text-xl text-slate-800">{name}</h3>
            <p className="text-slate-600 text-sm sm:text-base mt-1">{description}</p>
        </div>
